...
const store = new Vuex.Store({
  ...
  actions: {
    ...
    next(context,channel) {
      if(channel){
        fetch('https://jirenguapi.applinzi.com/fm/getSong.php?'+ channel).then(response=>response.json()).then(data=>{
          context.commit('next',data.song[0])
          context.commit('setChannel',channel)
        }).catch(e=>{
          console.log(e)
          context.dispatch('next',channel)
        })
      }else{
        fetch('https://jirenguapi.applinzi.com/fm/getSong.php?'+ context.state.channel).then(response=>response.json()).then(data=>{
          context.commit('next',data.song[0])
        }).catch(e=>{
          console.log(e)
          context.dispatch('next',channel)
